...
const out = await pg.evaluate(() => {
  const W = document.documentElement.clientWidth, res = [];
  for (const el of document.querySelectorAll('body *')) {
    const r = el.getBoundingClientRect();
    if (r.right > W + 0.5 || r.left < -0.5) res.push(`${el.tagName}.${String(el.getAttribute('class')||'').slice(0,60)} l=${Math.round(r.left)} r=${Math.round(r.right)}`);
    if (res.length >= 10) break;
  }
  return { sw: document.documentElement.scrollWidth, W, res };
});
