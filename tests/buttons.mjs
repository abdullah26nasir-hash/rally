...
const state = () => p.evaluate(() => [location.href, document.body.innerText, window.__shared || 0, document.querySelectorAll('[aria-checked=true],[aria-expanded=true],[role=dialog]').length, document.querySelector('[aria-pressed=true]')?.textContent?.trim().slice(0,20), scrollY, document.activeElement?.id].join('|'));
for (const r of routes) {
  ...
  for (let i = 0; i < limit; i++) {
    ...
    const current = (await el.getAttribute('aria-current')) === 'page' || (await el.getAttribute('aria-pressed')) === 'true' || (await el.getAttribute('aria-selected')) === 'true' || (await el.getAttribute('aria-checked')) === 'true'; // already active
