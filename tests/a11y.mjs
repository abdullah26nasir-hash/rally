...
const only = process.env.A11Y_VIEW; const b = await launch(); const t = tally('a11y');
const states = [
  ...
  ['start 2', async (p) => { await p.goto(BASE + '/start', W); await p.getByLabel('Scout name').fill('Sam'); if (await p.getByRole('button', { name: 'No thanks' }).isVisible().catch(()=>false)) await p.getByRole('button', { name: 'No thanks' }).click(); await p.getByRole('button', { name: 'Continue' }).click(); }],
  ['start 3', async (p) => { await p.goto(BASE + '/start', W); await p.getByLabel('Scout name').fill('Sam'); if (await p.getByRole('button', { name: 'No thanks' }).isVisible().catch(()=>false)) await p.getByRole('button', { name: 'No thanks' }).click(); await p.getByRole('button', { name: 'Continue' }).click(); await p.getByRole('button', { name: 'Got it' }).click(); }],
  ['empty home', async (p) => { await p.goto(BASE + '/start', W); await p.getByLabel('Scout name').fill('Sam'); if (await p.getByRole('button', { name: 'No thanks' }).isVisible().catch(()=>false)) await p.getByRole('button', { name: 'No thanks' }).click(); await p.getByRole('button', { name: 'Continue' }).click(); await p.getByRole('button', { name: 'Got it' }).click(); await p.getByRole('button', { name: 'Make my first call' }).click(); await p.goto(BASE + '/', W); }],
  ...
  ['league join error', async (p) => { await p.goto(BASE + '/leagues', W); await p.getByLabel('League code').fill('ZZZZZZ'); await p.getByRole('button', { name: 'Join league' }).click(); await p.getByRole('alert').waitFor({timeout:5000}).catch(()=>{}); }],
