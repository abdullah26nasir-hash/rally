...
export function EventIcons({ w }: { w: WeekStat }) {
  ...
  if (w.moment) items.push({ icon: Sparkle, label: w.moment.toLowerCase(), tone: 'text-stamp-deep' });
  ...
  return (
    <span className="inline-flex items-center gap-1.5 min-w-0" title={sentence}>
      ...
      {w.moment && <span aria-hidden className="text-[12px] font-semibold text-stamp-deep truncate">{w.moment}</span>}
    </span>
  );
}
