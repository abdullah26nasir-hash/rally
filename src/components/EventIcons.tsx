...
export function EventIcons({ w }: { w: WeekStat }) {
  if (!w.minutes) return <span className="text-newsprint">Did not play</span>;
  ...
  if (w.moment) items.push({ icon: Sparkle, label: w.moment.toLowerCase(), tone: 'text-flare' });
  ...
  return (
    <span className="inline-flex items-center gap-1.5 min-w-0" title={sentence}>
      ...
      <span aria-hidden className="inline-flex items-center gap-0.5 font-mono text-[12px] text-newsprint"><Clock3 size={12} />{w.minutes}'</span>
      {items.map((x, i) => <x.icon key={i} aria-hidden size={15} strokeWidth={2.2} className={x.tone ?? 'text-paper'} />)}
      {w.moment && <span aria-hidden className="text-[12px] font-semibold text-flare truncate">{w.moment}</span>}
    </span>
  );
}
