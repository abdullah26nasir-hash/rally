...
export function How() {
  ...
  return (
    <div className="max-w-[760px]">
      <Link to="/" className="inline-flex min-h-11 items-center font-semibold text-newsprint hover:text-paper">← Rallycademy</Link>
      ...
      <div className="grid gap-6">
        <Card className="p-5 sm:p-6"><h2 className="display text-[28px]">Your list</h2><ul className="mt-3 grid gap-2 list-disc pl-5 text-newsprint"><li>Five under-21 players, one per club.</li><li>Change freely until your first deadline. After that you get one swap a week.</li><li>A player scores for you from the first gameweek after you scout him.</li></ul></Card>
        ...
        <Card className="overflow-hidden"><h2 className="display text-[28px] px-5 sm:px-6 pt-5">Early call</h2><p className="px-5 sm:px-6 mt-1 text-newsprint">Locked the moment you scout a player, based on the share of scouts who had him. It multiplies every point he scores for you. Swap him out and it's gone.</p>
        ...
        <p className="text-[14px] text-newsprint">Preview season: players, clubs, results and scout numbers are fictional and simulated. Rallycademy is free to play. You play for bragging rights.</p>
        {mode !== 'new' && <Card className="p-5 sm:p-6">
          <h2 className="display text-[28px]">{mode === 'sample' ? 'Play your own season' : 'Start over'}</h2>
          <p className="mt-1 text-newsprint">{mode === 'sample' ? 'Leave the sample season and scout your own five.' : 'Clears your list, receipts, stamps and leagues on this device.'}</p>
      ...
      <Sheet open={confirm} onOpenChange={setConfirm} title="Start over?" description="Your list, receipts, stamps and leagues on this device will be cleared. This can't be undone.">
        <div className="grid gap-3 p-5 pt-2">
          <Button className="bg-flare hover:bg-flare/90" onClick={() => { reset(); setConfirm(false); nav('/start'); }}>Clear and start over</Button>
