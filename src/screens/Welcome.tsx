  ...
  return (
    <div className="min-h-dvh">
      <header className="max-w-[1180px] mx-auto flex items-center justify-between px-5 sm:px-8 h-16">
        <Wordmark />
        <button onClick={() => nav('/how')} className="text-[15px] font-medium text-newsprint hover:text-paper min-h-11 px-2">How scoring works</button>
      </header>
      ...
      <section className="max-w-[1180px] mx-auto px-5 sm:px-8 pt-6 sm:pt-12 pb-16 grid grid-cols-[minmax(0,1fr)] lg:grid-cols-[minmax(0,1.15fr)_minmax(0,1fr)] gap-12 lg:gap-8 items-center">
        <div>
          ...
          <h1 className="display text-[54px] min-[360px]:text-[64px] sm:text-[88px] lg:text-[112px] leading-[0.86]">
            Call them<br />before they<br /><span className="hl text-ink">blow up.</span>
          </h1>
          <p className="mt-6 text-[18px] sm:text-[20px] max-w-[34ch] text-newsprint">
          ...
          <p className="mt-4 text-[14px] text-newsprint">No sign-up needed to try it. About five minutes a week.</p>
        </div>
        <div className="lg:pl-6">
          <div className="lg:scale-[1.1] lg:-rotate-2 lg:origin-center lg:py-6"><Receipt pick={heroPick} scoutName="you" animate /></div>
          <p className="mt-5 text-center text-[13px] text-newsprint font-mono">Straight from the sample season</p>
        </div>
      </section>

      <section className="bg-surface-1 border-y border-hairline">
        <div className="max-w-[1180px] mx-auto px-5 sm:px-8 py-16 lg:py-20">
          ...
          <ol className="mt-10 grid md:grid-cols-3 gap-8 md:gap-10">
            {[
              ...
              <li key={t} className="border-t-2 border-hairline pt-4">
                <div className="font-mono text-[13px] text-newsprint">Step {i + 1}</div>
                <h3 className="display text-[30px] mt-1">{t}</h3>
                <p className="mt-2 text-newsprint">{d}</p>
              </li>
            ))}
          </ol>
        </div>
      ...
      <section className="max-w-[1180px] mx-auto px-5 sm:px-8 py-16 lg:py-20">
        ...
        <p className="mt-3 text-[18px] text-newsprint max-w-[46ch]">Check your slip, make your one swap, collect your stamps. These are real screens from the sample season.</p>
        <ul tabIndex={0} aria-label="App screens" className="focus-visible:outline-2 focus-visible:outline-flare mt-10 -mx-5 px-5 sm:mx-0 sm:px-0 flex sm:grid sm:grid-cols-3 gap-5 lg:gap-8 overflow-x-auto sm:overflow-visible no-scrollbar snap-x snap-mandatory scroll-px-5 pt-2 pb-8 -mb-6">
          {[['week', 'This week', 'Your gameweek slip and how far clear you are of your mates.'], ['scout', 'Scout', 'Who is breaking out, who nobody has found yet.'], ['stamps', 'Stamp book', 'A stamp for every call that comes good. Your scout level rises with them.']].map(([k, t, d]) => (
            <li key={k} className="snap-start shrink-0 w-[70vw] max-w-[300px] sm:w-auto sm:max-w-none">
              <div className="rounded-[28px] bg-surface-2 border border-card-edge p-2 shadow-md"><img src={`/screens/${k}.webp`} alt={`${t} screen from the sample season`} width={390} height={844} className="block w-full h-auto rounded-[21px]" /></div>
              <h3 className="display text-[26px] mt-4">{t}</h3>
              <p className="mt-1 text-newsprint">{d}</p>
            </li>
          ))}
        </ul>
      </section>
      ...
      <section className="max-w-[1180px] mx-auto px-5 sm:px-8 py-16 lg:py-20 grid lg:grid-cols-2 gap-10 items-start">
        <div>
          ...
          <p className="mt-4 text-[18px] text-newsprint max-w-[42ch]">Your early-call multiplier is locked the moment you scout a player, based on how many scouts had him. Pick him at 0.8% and every point he scores counts triple for you, all season.</p>
        </div>
        <div className="bg-surface-1 rounded-[8px] shadow-md overflow-hidden">
          <table className="w-full text-left">
            <caption className="sr-only">Early call multipliers</caption>
            <thead><tr className="border-b border-hairline text-[13px] font-mono text-newsprint"><th className="font-normal px-5 py-3">When you scouted him</th><th className="font-normal px-5 py-3 text-right">His points count</th></tr></thead>
            <tbody>
      ...
      <footer className="max-w-[1180px] mx-auto px-5 sm:px-8 pb-12 text-[14px] text-newsprint border-t border-hairline pt-6 flex flex-col sm:flex-row gap-2 justify-between">
